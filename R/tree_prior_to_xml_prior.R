...
tree_prior_to_xml_prior <- function(
  ...
  if (is_bd_tree_prior(tree_prior)) {
    text <- c(text, bd_tree_prior_to_xml_prior(tree_prior))
  } else if (is_cbs_tree_prior(tree_prior)) {
    # Nothing to do
  } else if (is_ccp_tree_prior(tree_prior)) {
    text <- c(text, ccp_tree_prior_to_xml_prior(tree_prior))
  } else if (is_cep_tree_prior(tree_prior)) {
    text <- c(text, cep_tree_prior_to_xml_prior(tree_prior))
  } else {
    testit::assert(beautier::is_yule_tree_prior(tree_prior))
    text <- c(text, yule_tree_prior_to_xml_prior(tree_prior))
  }
  text
}
