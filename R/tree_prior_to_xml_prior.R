...
tree_prior_to_xml_prior <- function(
  ...
  if (beautier::is_bd_tree_prior(tree_prior)) {
    text <- c(text, bd_tree_prior_to_xml_prior(tree_prior)) # internal function
  } else if (beautier::is_cbs_tree_prior(tree_prior)) {
    # Nothing to do
  } else if (beautier::is_ccp_tree_prior(tree_prior)) {
    text <- c(text, ccp_tree_prior_to_xml_prior(tree_prior)) # internal function
  } else if (beautier::is_cep_tree_prior(tree_prior)) {
    text <- c(text, cep_tree_prior_to_xml_prior(tree_prior)) # internal function
  } else {
    testit::assert(beautier::is_yule_tree_prior(tree_prior))
    text <- c(text, yule_tree_prior_to_xml_prior(tree_prior)) # internal function
  }
  text
}
