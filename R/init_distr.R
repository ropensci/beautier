...
init_distr <- function(
  ...
  if (is_beta_distr(distr)) { # nolint beautier function
    ...
    if (is_one_na(distr$mu$id)) {
      distr$mu$id <- param_id
      param_id <- param_id + 1
    }
    if (is_one_na(distr$scale$id)) {
    ...
    if (is_one_na(distr$m$id)) {
      distr$m$id <- param_id
      param_id <- param_id + 1
    }
    if (is_one_na(distr$s$id)) {
      distr$s$id <- param_id
    }
  ...
  testit::assert(!is_one_na(distr$id))
  distr
}
