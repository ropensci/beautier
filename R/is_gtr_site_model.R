...
is_gtr_site_model <- function(
  ...
  if (!is_param(x$rate_ac_param)) return(FALSE)
  if (!"rate_ag_param" %in% names(x)) return(FALSE)
  if (!is_param(x$rate_ag_param)) return(FALSE)
  if (!"rate_at_param" %in% names(x)) return(FALSE)
  if (!is_param(x$rate_at_param)) return(FALSE)
  if (!"rate_cg_param" %in% names(x)) return(FALSE)
  if (!is_param(x$rate_cg_param)) return(FALSE)
  if (!"rate_gt_param" %in% names(x)) return(FALSE)
  if (!is_param(x$rate_gt_param)) return(FALSE)
  TRUE
}
