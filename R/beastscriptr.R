...
beast_scriptr_operators <- function(
  ...
  if (fixed_crown_age == FALSE) {
    text <- c(text, paste0("    <operator id=\"SubtreeSlide.t:", filename_base,
      ...
      "\" weight=\"15.0\"/>"))                                          # nolint (as this is no absolute path)
    text <- c(text, "")
    ...
    text <- c(text, paste0("    <operator id=\"wide.t:", filename_base,
      ...
      "\" weight=\"3.0\"/>"))                                           # nolint (as this is no absolute path)
    text <- c(text, "")
  ...
  if (tree_prior == "birth_death") {
    text <- c(text, paste0("    <operator id=\"BirthRateScaler.t:",
      ...
      filename_base, "\" scaleFactor=\"0.75\" weight=\"3.0\"/>"))     # nolint (as this is no absolute path)
    text <- c(text, "")
...
beast_scriptr_distribution <- function(
  ...
  if (tree_prior == "birth_death") {
    ...
    text <- c(text, paste0(
      ...
      filename_base, "\"/>"))                                         # nolint (as this is no absolute path)
  ...
  if (tree_prior == "birth_death") {
    ...
    text <- c(text,
      paste0(
        ...
        "name=\"distr\" upper=\"1000.0\"/>"                           # nolint (as this is no absolute path)
        )
      )
    text <- c(text, "            </prior>")
  ...
  text <- c(
      ...
      paste0(
        ...
        "spec=\"util.CompoundDistribution\">"                         # nolint (as this is no absolute path)
      )
    )
  ...
  text <- c(text, paste0("                    <substModel id=\"JC69.s:",
    filename_base, "\" spec=\"JukesCantor\"/>"))                      # nolint (as this is no absolute path)
...
beast_scriptr_init <- function(
  ...
  if (!ribir::is_phylogeny(initial_phylogeny)) {
    text <- c(text, paste0("    <init id=\"RandomTree.t:", filename_base,
      ...
      sep = ""))
    text <- c(text, paste0("        <populationModel id=\"ConstantPopulation0.t:",
...
beast_scriptr_loggers <- function(
  ...
  if (tree_prior == "birth_death") {
    ...
    text <- c(text, paste0("        <parameter idref=\"popSize.t:",
      filename_base, "\" name=\"log\"/>"))                            # nolint (as this is no absolute path)
  ...
  text <- c(text, paste0("        <log id=\"ESS.0\" spec=\"util.ESS\" ",
    "arg=\"@posterior\"/>"))                                          # nolint (as this is no absolute path)
  text <- c(text, "        <log idref=\"likelihood\"/>")                        # nolint (as this is no absolute path)
  text <- c(text, "        <log idref=\"prior\"/>")                             # nolint (as this is no absolute path)
  text <- c(text, "    </logger>")
  text <- c(text, "")
...
beast_scriptr_state <- function(
  ...
  if (!ribir::is_phylogeny(initial_phylogeny)) {
    ...
    text <- c(text, paste0("        <tree id=\"Tree.t:", filename_base,
      "\" name=\"stateNode\">"))
    text <- c(text, paste0("            <taxonset id=\"TaxonSet.", filename_base,
      "\" spec=\"TaxonSet\">"))
    text <- c(text, paste0("                <alignment idref=\"", filename_base,
      "\"/>"))   # nolint (as this is not an absolute path)
