test_that("tipdates file must be used in the created file", {
  ...
  if (1 == 2) {
    ...
    testthat::expect_equal(
      1,
      length(stringr::str_subset(readr::read_lines("tipdate.tsv"), "2013.91666666667"))
    )

    create_beast2_input_file(
      input_filename = fasta_file,
      output_filename = output_xml,
      site_model = site_model,
      clock_model = clock_model,
      tree_prior = tree_prior,
      mcmc = mcmc
    ...
    text <- readr::read_lines(output_xml)

    testthat::expect_equal(
      1,
      length(stringr::str_subset(text, "2013.91666666667"))
    )
  }
