...
test_that("initialize RLN clock model", {
  ...
  expect_true(is_rln_clock_model(before[[1]]))
  expect_true(!are_init_clock_models(before))
  after <- init_clock_models(
    before,
    fasta_filenames = fasta_filenames
  )
  testthat::expect_true(is_rln_clock_model(after[[1]]))
  testthat::expect_true(are_init_clock_models(after))
...
test_that("initialize strict clock model", {
  ...
  expect_true(is_strict_clock_model(before[[1]]))
  expect_true(!are_init_clock_models(before))
  after <- init_clock_models(
    before,
    fasta_filenames = fasta_filenames
